...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getCalculators() {
    ...
    const response = await prisma.calculator
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'calculator'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createCalculator() {
    ...
    const body = { ...req.body };
    if (body?.locker?.length > 0) {
      const create_locker = body.locker;
      body.locker = {
        create: create_locker,
      };
    } else {
      delete body.locker;
    }
